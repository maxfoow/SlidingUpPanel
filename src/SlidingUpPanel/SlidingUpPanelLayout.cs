...
namespace Cheesebaron.SlidingUpPanel {
	public class SlidingUpPanelLayout : ViewGroup {
		...
		private const float CollapseOffset = 1.0f;
		private const float ExpandOffset = 0.0f;
		...
		public event SlidingUpPanelEventHandler PanelHidden;
		...
		public SlideState SlideState {
			...
			set {
				if ((!_firstLayout && _slideableView == null) || value == _slideState)
					return;
				if (_firstLayout) {
					_slideState = value;
				}
					...
					if (_slideState == SlideState.Hidden) {
						_slideableView.Visibility = ViewStates.Visible;
						RequestLayout();
					}
					switch (value) {
						case SlideState.Expanded:
							SmoothSlideTo(ExpandOffset, 0);
							break;
						case SlideState.Collapsed:
							SmoothSlideTo(CollapseOffset, 0);
							break;
						case SlideState.Anchored:
							SmoothSlideTo(_anchorPoint, 0);
							break;
						case SlideState.Hidden:
							var top = ComputePanelTopPosition(CollapseOffset) + (_isSlidingUp ? +PanelHeight : -PanelHeight);
							SmoothSlideTo(ComputeSlideOffset(top), 0);
							break;
						default:
		...
		public SlidingUpPanelLayout(Context context, IAttributeSet attrs, int defStyle) : base(context, attrs, defStyle) {
			...
			if (attrs != null) {
				...
				if (ta.Length() > 0) {
					...
					_minFlingVelocity = ta.GetInt(Resource.Styleable.SlidingUpPanelLayout_flingVelocity, DefaultMinFlingVelocity);
		...
		private void OnPanelHidden(View panel) {
			PanelHidden?.Invoke(this, new SlidingUpPanelEventArgs {
				Panel = panel
			});
			SendAccessibilityEvent(EventTypes.WindowStateChanged);
		}
		...
		protected override void OnMeasure(int widthMeasureSpec, int heightMeasureSpec) {
			...
			for (var i = 0; i < ChildCount; i++) {
				...
				if (i == 1) {
					...
					_canSlide = true;
					if (_slideableView.Visibility != ViewStates.Visible) {
						_slideState = SlideState.Hidden;
					}
				}
				else {
					// content
					if (!OverlayContent && _slideState != SlideState.Hidden)
						height -= panelHeight;
				}
			...
			if (_firstLayout) {
				switch (_slideState) {
					...
					case SlideState.Collapsed:
						...
						break;
					case SlideState.Hidden:
						var top = ComputePanelTopPosition(1.0f) + (_isSlidingUp ? +PanelHeight : -PanelHeight);
						_slideOffset = ComputeSlideOffset(top);
						break;
				}
			}
		...
		private int ComputePanelTopPosition(float slideOffset) {
			...
			int slidingViewHeight = _slideableView != null ? _slideableView.MeasuredHeight : 0;
			int slidePixelOffset = (int) (slideOffset * _slideRange);
			// Compute the top of the panel if its collapsed
			return _isSlidingUp ? MeasuredHeight - PaddingBottom - PanelHeight - slidePixelOffset : PaddingTop - slidingViewHeight + PanelHeight + slidePixelOffset;
		}

		private float ComputeSlideOffset(int topPosition) {
			// Compute the panel top position if the panel is collapsed (offset 0)
			var topBoundCollapsed = ComputePanelTopPosition(1.0f);

			// Determine the new slide offset based on the collapsed top position and the new required
			// top position
			return (_isSlidingUp ? (float) (topPosition) / _slideRange : (float) (topPosition - topBoundCollapsed) / _slideRange);
		}
		...
		private bool IsDragViewUnder(int x, int y) {
			...
			return screenX >= viewLocation[0] && screenX < viewLocation[0] + dragView.Width && screenY >= viewLocation[1] && screenY < viewLocation[1] + dragView.Height;
		}

		public bool CollapsePane() {
			if (_firstLayout || SmoothSlideTo(1.0f, 0))
				return true;
			return false;
		}
		...
		public bool ExpandPane(float slideOffset) {
			if (!PaneVisible)
				ShowPane();
			return _firstLayout || SmoothSlideTo(slideOffset, 0);
		}
		...
		private void OnPanelDragged(int newTop) {
			_slideOffset = _isSlidingUp ? (float) (newTop - SlidingTop) / _slideRange : (float) (SlidingTop - newTop) / _slideRange;
		...
		private bool SmoothSlideTo(float slideOffset, int velocity) {
			if (!_canSlide) return false;

			var panelTop = ComputePanelTopPosition(slideOffset);

			if (!_dragHelper.SmoothSlideViewTo(_slideableView, _slideableView.Left, panelTop)) {
				return false;
			}
		...
		private class DragHelperCallback : ViewDragHelper.Callback {
			...
			public override void OnViewDragStateChanged(int state) {
				...
				if (_panelLayout._dragHelper.ViewDragState == ViewDragHelper.StateIdle) {
					_panelLayout._slideOffset = _panelLayout.ComputeSlideOffset(_panelLayout._slideableView.Top);
					...
					else if (_panelLayout._slideOffset > 1) {
						_panelLayout._slideState = SlideState.Hidden;
						_panelLayout._slideableView.Visibility = ViewStates.Invisible;
						_panelLayout.OnPanelHidden(_panelLayout._slideableView);
					}
					else if (FloatNearlyEqual(_panelLayout._slideOffset, 1)) {
						if (_panelLayout._slideState != SlideState.Collapsed) {
			...
			public override void OnViewReleased(View releasedChild, float xvel, float yvel) {
				var top = _panelLayout._isSlidingUp ? _panelLayout.SlidingTop : _panelLayout.SlidingTop - _panelLayout._slideRange;
				...
				if (!FloatNearlyEqual(_panelLayout._anchorPoint, 0)) {
					...
					if (_panelLayout._isSlidingUp) {
						...
						anchoredTop = _panelLayout._panelHeight - (int) (_panelLayout._anchorPoint * _panelLayout._slideRange);
					...
					else if (FloatNearlyEqual(yvel, 0) && _panelLayout._slideOffset < (1f + anchorOffset) / 2 && _panelLayout._slideOffset >= anchorOffset / 2)
		...
		public class SavedState : BaseSavedState {
			...
			public SavedState(IParcelable superState, SlideState item) : base(superState) {
				State = item;
			}
